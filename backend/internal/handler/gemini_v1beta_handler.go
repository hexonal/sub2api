...
func (h *GatewayHandler) GeminiV1BetaListModels(c *gin.Context) {
	...
	if !hasForcePlatform && (apiKey.Group == nil || service.ResolveGroupPlatform(c.Request.Context(), apiKey.Group.Platform) != service.PlatformGemini) {
...
func (h *GatewayHandler) GeminiV1BetaModels(c *gin.Context) {
	...
	if !middleware.HasForcePlatform(c) {
		if apiKey.Group == nil || service.ResolveGroupPlatform(c.Request.Context(), apiKey.Group.Platform) != service.PlatformGemini {
			googleError(c, http.StatusBadRequest, "API key group platform is not gemini")
			return
		}
	}
