...
describe('UseKeyModal', () => {
  ...
  it('renders Entrox CLI install methods and login command', async () => {
    ...
    expect(codeBlocks[1]).toBe('curl -fsSL https://entrox.996icu.wiki/install | bash')
    expect(codeBlocks[1]).not.toContain('HOMEBREW_NO_AUTO_UPDATE')
    expect(codeBlocks[1]).not.toContain('irm https://entrox.996icu.wiki/install.ps1 | iex')
    expect(codeBlocks[2]).toContain('irm https://entrox.996icu.wiki/install.ps1 | iex')
    expect(codeBlocks[2]).toContain('Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser')
    expect(codeBlocks[2]).toContain('Invoke-RestMethod -Uri https://get.scoop.sh | Invoke-Expression')
    expect(codeBlocks[2]).toContain('scoop bucket add entrox https://github.com/hexonal/scoop-entrox')
    expect(codeBlocks[2]).toContain('scoop install entrox')
    expect(codeBlocks[2]).not.toContain('curl -fsSL https://entrox.996icu.wiki/install | bash')
    expect(codeBlocks[3]).toBe('entrox login')
  ...
  it('keeps official CN Entrox install commands concise', async () => {
    ...
    expect(codeBlocks[1]).toBe('curl -fsSL https://entrox.996icu.wiki/install | bash')
    expect(codeBlocks[2]).toContain('irm https://entrox.996icu.wiki/install.ps1 | iex')
    expect(codeBlocks[2]).not.toContain('scoop config proxy')
    expect(codeBlocks[2]).not.toContain('你的代理端口')
    expect(codeBlocks[2]).not.toContain('127.0.0.1:7890')
  })
